...
<div class="p-4 border-sky-200 border-1 rounded-xl w-full sm:w-fit sm:max-w-[400px]">
    ...
    <p v-if="service.phones.length" class="flex gap-1 mb-3">
        <a v-for="(phone, i) in service.phones" :key="i" :href='`tel:${phone.phone}`' class="underline text-blue-800">{{phone.number}}</a> 
    </p>
    ...
    <p v-if="service.socialMedia.length" class="flex flex-col gap-1 mt-3">
        <a v-for="(sm) in service.socialMedia" :key="sm.smKey" :href="sm.link" class=" text-blue-800 underline flex items-center gap-1 mb-3">
            
            <svg v-if="sm.smKey === 'instagram'"  xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="24" height="24" viewBox="0 0 48 48">
            ...
            <svg v-else-if="sm.smKey === 'facebook'" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="24" height="24" viewBox="0 0 48 48">
